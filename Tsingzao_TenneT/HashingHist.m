function [f BlkIdx] = HashingHist(TentNet,ImgIdx,OutImg)
...
% OutImg  TentNet filter output before the last stage (cell structure)
% ========= OUTPUT ===========
% f       TentNet features (each column corresponds to feature of each image)
% BlkIdx  index of local block from which the histogram is compuated
% ============================

NumImg = max(ImgIdx);
f = cell(NumImg,1);
map_weights = 2.^((TentNet.NumFilters(end)-1):-1:0); % weights for binary to decimal conversion

for Idx = 1:NumImg
  
    Idx_span = find(ImgIdx == Idx);
    NumOs = length(Idx_span)/TentNet.NumFilters(end); % the number of "O"s
    Bhist = cell(NumOs,1);
    
    for i = 1:NumOs 
        
        T = 0;
        ImgSize = size(OutImg{Idx_span(TentNet.NumFilters(end)*(i-1) + 1)});
        for j = 1:TentNet.NumFilters(end)
            T = T + map_weights(j)*Heaviside(OutImg{Idx_span(TentNet.NumFilters(end)*(i-1)+j)}); 
            % weighted combination; hashing codes to decimal number conversion
            
            OutImg{Idx_span(TentNet.NumFilters(end)*(i-1)+j)} = [];
        end
        
        
        if isempty(TentNet.HistBlockSize)
            NumBlk = ceil((TentNet.ImgBlkRatio - 1)./TentNet.BlkOverLapRatio) + 1;
            HistBlockSize = ceil(size(T)./TentNet.ImgBlkRatio);
            ...
            OverLapinPixel),(0:2^TentNet.NumFilters(end)-1)')); 
        else 
            
            stride = round((1-TentNet.BlkOverLapRatio)*TentNet.HistBlockSize); 
            blkwise_fea = sparse(histc(im2col_general(T,TentNet.HistBlockSize,...
              stride),(0:2^TentNet.NumFilters(end)-1)')); 
            % calculate histogram for each local block in "T"
            
           if ~isempty(TentNet.Pyramid)
                x_start = ceil(TentNet.HistBlockSize(2)/2);
                y_start = ceil(TentNet.HistBlockSize(1)/2);
                x_end = floor(ImgSize(2) - TentNet.HistBlockSize(2)/2);
                y_end = floor(ImgSize(1) - TentNet.HistBlockSize(1)/2);
                
                sam_coordinate = [...
                    kron(x_start:stride:x_end,ones(1,length(y_start:stride: y_end))); 
                    kron(ones(1,length(x_start:stride:x_end)),y_start:stride: y_end)];               
                
                blkwise_fea = spp(blkwise_fea, sam_coordinate, ImgSize, TentNet.Pyramid)';
